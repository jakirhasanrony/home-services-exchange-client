...
import ProviderSpecificServiceCard from "./ProviderSpecificServiceCard";
...
const ServiceDetailsCard = () => {
    ...
    const [providerSpecificServices, setProviderSpecificServices] = useState([]);
    const url2 = `https://home-services-exchange-server.vercel.app/providerSpecificServices?service_provider_email=${service_provider_email}`;
    useEffect(() => {
        fetch(url2)
            .then(res => res.json())
            .then(data => setProviderSpecificServices(data));


    }, [url2])
    console.log(providerSpecificServices);
    ...
    return (
        <>
            <div className="mt-[-8px]" style={{ background: 'linear-gradient(to right, #c2e0ff, #f0f0f0)' }}>
                ...
                <div className=" shadow-lg mx-auto p-8  grid grid-cols-1 justify-center items-center" style={{ background: 'linear-gradient(to right, #c2e0ff, #f0f0f0)' }}>
                    <div>
                ...
                <div>
                    <div className="">
                        <h2 className="text-2xl p-10 text-amber-700 font-bold text-center">Total Services From This Provider</h2>
                    ...
                    <div className="p-8 flex flex-wrap gap-4 justify-center items-center">
                        {
                            providerSpecificServices.map(providerSpecificService => <ProviderSpecificServiceCard
                                key={providerSpecificService._id}
                                providerSpecificService={providerSpecificService}
                            ></ProviderSpecificServiceCard>)
                        }
                    </div>
                </div>
